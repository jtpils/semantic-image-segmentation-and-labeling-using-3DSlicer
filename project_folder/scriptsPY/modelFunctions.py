...
class StackedUnet(nn.Module):
    def __init__(self):
        ...
        self.decoderUnet = UNet(3,1)


    def forward(self,x):
        yy = self.encoderUnet(x) # label predictions are output
        xx = self.decoderUnet(yy) # reconstructed signal is output
...
def lossfunc(img,labels,decimg,wtperclass,ignore_index=-100,alpha=0.9):
        
    # loss_ce = F.cross_entropy(img,labels,weight=wtperclass,reduction='none')
    # NOTE: We are using ignore_index = -100 to serve as a mask for labels we do not have
    loss_ce = F.cross_entropy(img,labels,ignore_index=ignore_index,reduction='none') # not using wtperclass for the stacked unet

    total_loss = alpha*torch.mean(loss_ce) + (1-alpha)*F.mse_loss(decimg,img)
...
def train(epoch, model, optimizer, train_loader, cuda_available, wtperclass):
    ...
    for batch_idx, (image, labels) in enumerate(train_loader):
        optimizer.zero_grad()
        pred0, imgdec = model(image) # model outputs label predictions and decoded/reconstucted image
...
def test(model, test_loader, cuda_available, wtperclass):
    ...
    for image, labels in test_loader:
        if cuda_available:
            image, labels = image.cuda(), labels.cuda()
        pred0,imgdec = model(image)
        loss0 = lossfunc(pred0,labels,image,wtperclass)
        
        test_loss = test_loss + loss0
        pred = pred0.max(1)[1] # get the index of the max log-probability

        correct = correct + pred.eq(labels).sum()
        
        dim2=np.shape(labels)[1]
        dim3=np.shape(labels)[2]

    test_loss = test_loss.item()
    test_loss /= len(test_loader) # loss function already averages over batch size
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
        test_loss, correct.item(), len(test_loader.dataset)*dim2*dim3,
        100. * correct / (len(test_loader.dataset)*dim2*dim3)))
...
def predict(model, X_train, ind, labelssave):
    ...
    for i in ind: 
         ...
         out = out.max(1)[1] # get the index of the max log-probability
...
class RandomCropAroundPosition(object): # RK added this
    ...
    def __init__(self, size_img, cx, cy, blobwd=112, patchwd=256):
        self.size_img = size_img
        self.th, self.tw = size_img
        self.cx = cx
        self.cy = cy
...
def TrainTestSplit(labelpath, seed=None, numbertrainingimg=3, 
    ...
    y_train = y_filenames
    X_valid = np.delete(X_filenames, indeces) # will get the files from index startvalid to the end unless you insert stopvalid here
